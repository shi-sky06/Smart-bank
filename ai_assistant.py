...
# NEW: plural / "listing" forms of a transaction type. Saying "transfers"
# or "international transfers" (as opposed to singular "transfer") reads
# as "show me a list of them" rather than "I want to make one", so these
# alone are enough to treat the message as a lookup even with no other
# query verb, date, or name attached.
PLURAL_TYPE_TRIGGERS = [
    "international transfers", "transfers", "withdrawals", "deposits",
]
...
# ------------------------------------------------------------
# NEW: typo-tolerant word matching. A short helper used to catch
# common misspellings ("lagest", "witdraw", "depsit", "smalllest")
# without needing to hand-maintain a list of every possible typo.
# ------------------------------------------------------------

def _fuzzy_word_in_set(text: str, word_set, threshold: float = 0.82) -> bool:
    """True if any word in `text` is a close (typo-tolerant) match to
    any word in `word_set`. Only compares words of similar length to
    keep it cheap and to avoid false positives on short words."""
    text_words = re.findall(r"[a-z]+", text)
    for w in text_words:
        if len(w) < 4:
            continue  # too short to fuzzy-match safely (e.g. "to" vs "no")
        for target in word_set:
            if abs(len(w) - len(target)) > 2:
                continue
            if SequenceMatcher(None, w, target).ratio() >= threshold:
                return True
    return False
...
def _detect_extremum(text: str, has_type: bool = False):
    ...
    if has_type:
        ...
        # exact match first, then typo-tolerant fallback (e.g. "lagest")
        if words & EXTREMUM_WORDS_DESC or _fuzzy_word_in_set(text, EXTREMUM_WORDS_DESC):
            return "desc"
        if words & EXTREMUM_WORDS_ASC or _fuzzy_word_in_set(text, EXTREMUM_WORDS_ASC):
            return "asc"
...
def _handle_transaction_query(message: str):
    ...
    # NEW: a plural type word ("transfers", "withdrawals", "deposits",
    # "international transfers") reads as "show me a list", so it's
    # enough on its own to treat the message as a lookup -- unlike a
    # bare singular ("transfer") which usually means "I want to make
    # one" and should stay routed to the action intent.
    has_plural_type = any(p in text for p in PLURAL_TYPE_TRIGGERS)
    ...
    # If a plural type word or an extremum word gave us the lookup
    # signal but we don't otherwise have any filters, fall back to
    # listing that type specifically instead of silently returning None.
    if type_is_lookup and not any([name, date, date_from, date_to, has_amount_range]) and not order and not mode:
        return _reply(get_transactions_filtered(types=txn_types))
...
def _handle_loan_query(message: str):
    ...
    if is_list:
        ...
        for loan_type, amount, duration, interest, status, remarks, applied_date in loans:
            emoji = "\U0001F7E2" if status == "Approved" else ("\U0001F534" if status == "Rejected" else "\U0001F7E1")
            lines.append(
                f"\u2022 {loan_type} \u2014 \u20b9{amount:,.2f} over {duration} months \u2014 "
                f"{status} {emoji} (applied {str(applied_date)[:10]})"
            )
        return _reply("\U0001F4CB Your loans:\n\n" + "\n".join(lines))
    ...
    if is_due:
        ...
        return _reply(
            f"\U0001F4C5 Based on your loan start date ({str(applied_date)[:10]}), your next "
    ...
    if is_owe:
        return _reply(
            f"\U0001F4B0 Your {loan_type} of \u20b9{amount:,.2f} has a total repayable amount of "
    ...
    if is_emi:
        return _reply(
            f"\U0001F4B3 Your estimated monthly EMI for your {loan_type} is "
            f"\u20b9{emi:,.2f} over {duration} months."
        )

    status_emoji = "\U0001F7E2" if status == "Approved" else "\U0001F534"
    reply = (
