...
class ManageUsersPage(ctk.CTkFrame):
    ...
    def __init__(self, master):
        ...
        # -------------------------
        button_row = ctk.CTkFrame(edit_card, fg_color="transparent")
        button_row.pack(pady=(15, 20))
    ...
    def load_user(self):
        ...
        self.email.insert(0, "" if user[5] is None else str(user[5]))
        self.balance.insert(0, "" if user[4] is None else str(user[4]))
    ...
    def delete_user(self):
        ...
        display_name = self.username.get().strip() or f"User ID {self.selected_user_id}"
 
        confirm = messagebox.askyesno(
            "Confirm Deletion",
            f"Are you sure you want to permanently delete '{display_name}'?\n\n"
            "This will also permanently delete all of their transaction "
            "history and loan applications.\n\n"
        ...
        try:
            # Clean up related records first so nothing is left orphaned.
            # Wrapped so a failure partway through rolls everything back
        ...
        messagebox.showinfo(
            "Deleted",
            f"'{display_name}' and all associated records have been deleted."
        )
